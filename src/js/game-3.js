...
class GameThreeView extends AbstractView {
	...
	onClick(index) {
		...
		if (this.gameState[`lives`] < MINIMUM_LIVES_NUMBER) {
			...
			const currentQuestionElement = createElement(getGameOneScreenTemplate(answers, this.gameState, QUESTIONS));
			const currenQuestionHeaderElement = createElement(getHeaderTemplate(this.gameState));

			showHeader(currenQuestionHeaderElement);
			showGameOneScreen(currentQuestionElement, this.gameState);
		}
	}
	...
	bind() {
		...
		gameThreeOptions.forEach((gameThreeOption, index) => {
			gameThreeOption.addEventListener(`click`, () => this.onClick(index));
		});
	}
}
// ////////////////
